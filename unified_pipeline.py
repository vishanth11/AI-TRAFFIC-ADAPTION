...
def run_pipeline(source: str, mode: str):
    ...
    frame_count = 0
    for g_result, e_result in zip(general_results, emergency_results):
        frame_count += 1

        # Collect emergency boxes for this frame
        emergency_boxes = []
        for box in e_result.boxes:
            if int(box.cls) == 0:  # emergency_vehicle class
                emergency_boxes.append(list(map(int, box.xyxy[0])))

        frame_detections = []

        if g_result.boxes.id is None:
            all_frames.append({"frame": frame_count, "detections": []})
            continue
        ...
        all_frames.append({"frame": frame_count, "detections": frame_detections})

        if frame_count % 30 == 0:
            print(f"\nFrame {frame_count} — {len(frame_detections)} objects"
                  + (f" | {len(emergency_boxes)} EMERGENCY" if emergency_boxes else ""))
            for d in frame_detections:
                tag = " *** EMERGENCY ***" if d["emergency"] else ""
                print(f"  ID={d['vehicle_id']:4d} | {d['type']:20s} | "
                      f"conf={d['confidence']:.2f} | "
                      f"pos=({d['position']['x']},{d['position']['y']}) | "
                      f"dir={d['direction']:7s} | speed={d['speed']} km/h{tag}")
...
if __name__ == "__main__":
    if len(sys.argv) < 3:
        print("Usage:")
        print("  Image: python unified_pipeline.py image <path>")
        print("  Video: python unified_pipeline.py video <path>")
        sys.exit(1)
